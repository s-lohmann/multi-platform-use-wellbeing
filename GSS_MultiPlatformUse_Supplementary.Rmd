...
# importing customized functions that overwrite the svycor function and a function that it draws on
# to also return bootstrap CIs, not just SEs for weighted correlation tests
source("GSS_helperfunctions.R")
...
cors.wt.out = matrix(
  sprintf("%.02f%s %s", cors.wt.all$cors, ifelse(cors.wt.all$p.values < .05, "*", ""), cors.wt.all$ci),
  dim(cors.wt.all$cors)[1], dim(cors.wt.all$cors)[2])

cors.wt.p = as.data.frame(rbind(
  cors.wt.all$p.values[-c(1:2), c(1:2)], 
  cors.wt.all$p.values[2, c(1:2)]))

cors.wt.ci = as.data.frame(rbind(
  cors.wt.all$ci[-c(1:2), c(1:2)], 
  cors.wt.all$ci[2, c(1:2)]))
...
cors.wt.out = as.data.frame(rbind(cors.wt.out[-c(1:2), c(1:2)],
                                  cors.wt.out[2, c(1:2)]))

colnames(cors.wt.out) = c("Online social contact", "Offline social contact")

cors.wt.out %>%
  mutate(Outcome = c(names(vars_outcome), "Offline social contact"),
         `Offline social contact` = 
           ifelse(Outcome == "Offline social contact", 
                  ...
                  ifelse(Outcome %in% c("Depression", "Depression (reduced scale)",
                         ...
                         "\\+", as.character(`Offline social contact`)))) %>%
